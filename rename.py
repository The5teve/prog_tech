...
while True:
    ...
    for j in range(int(numfile)): #делаем столько циклов, сколько файлов получили
        ...
        if file_name.find('_mails.txt')>1: #если нашло файл с именем _mails.txt:
            #print(file_name + ' первый принт')
            RSA_1.decrypt_file(file_name, int(file_name[0: file_name.find('_')]), int(file_name[file_name.find('_') + 1: file_name.find('_mails.txt')])) #декодируем по библиотеке RSA_1 файл
            os.rename(file_name, '_mails.txt') #переименовываем файл с адресами почт в _mails.txt
            if os.path.exists('_mails.txt'):
                new_zip.write('_mails.txt') #зап
            ...
            RSA_1.decrypt_file(file_name, int(file_name[0: file_name.find('_')]), int(file_name[file_name.find('_') + 1: file_name.find('.')])) #декодируем по библиотеке RSA_1 файл
    ...
    os.system('python3.8 mail_server.py')
